% Run a permutation test comparing the effects of load on slow-wave ERP
% amplitude.
%
...
for compi = 1:size(comparisons,1)
    ...
    D1 = grandAverage.(comparisons{compi,1});
    D2 = grandAverage.(comparisons{compi,2});
    ...
    cfg.alpha       = 0.05;
    cfg.latency     = [.650,1.45]; %time period we want to compare
    ...
    % Save the output
    saveName = ['erp_',comparisons{compi,1},'_',comparisons{compi,2}];
...
for compi = 1:size(comparisons,1)
    ...
    % Load the results
    loadName = ['erp_',comparisons{compi,1},'_',comparisons{compi,2}];
    ...
    formatSpec = '%s vs %s: pos. p = %.3f, pos. cd = %.2f, neg. p = %.3f, neg. cd = %.2f\n';
    fprintf(formatSpec,comparisons{compi,1},comparisons{compi,2}, input1,input2,input3,input4);
end