...
% Load data
load([pathData,'grandAverageL02vL04_erpcorrected_Slope.mat']);
load([pathData,'extras', filesep,'neighbours_GWM.mat']);

% Conditions to compare
comparisons = {'slope','L02','Delay','Base';...
    'slope','L04','Delay','Base';...
    'slope','L06','Delay','Base';...
    'offset','L02','Delay','Base';...
    'offset','L04','Delay','Base';...
    'offset','L06','Delay','Base'};
...
for compi = 1:size(comparisons,1)
    ...
    D1 = grandAverage.(comparisons{compi,3}).(comparisons{compi,2}).(comparisons{compi,1});
    D2 = grandAverage.(comparisons{compi,4}).(comparisons{compi,2}).(comparisons{compi,1});
    ...
    % Save the output
    saveName = [comparisons{compi,1},'_',comparisons{compi,2},'_',comparisons{compi,3},'_',comparisons{compi,4},'_erpcorrected'];
...
for compi = 1:size(comparisons,1)
    ...
    % Load the results
    loadName = [comparisons{compi,1},'_',comparisons{compi,2},'_',comparisons{compi,3},'_',comparisons{compi,4},'_erpcorrected'];
    ...
    formatSpec = '%s %s %s vs %s: pos. p = %.3f, pos. cd = %.2f, neg. p = %.3f, neg. cd = %.2f\n';
    fprintf(formatSpec,comparisons{compi,1},comparisons{compi,2}, comparisons{compi,3}, comparisons{compi,4}, input1,input2,input3,input4);
end