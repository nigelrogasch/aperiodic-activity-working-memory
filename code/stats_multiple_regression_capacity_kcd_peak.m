...
% and ERPs with working memory capacity extracting the peak channel.
%
...
% Load stats
loadName = ['correlation_capacity_kcd_',parameter,'_',loadIn];
load([pathStats,loadName,'.mat']);

% Extract the data
EEG_data = grandAverage.(loadIn).(parameter);
[~,chani] = max(stat.rho);
slopeMean = mean(EEG_data.individual(:,chani),2);
...
[~,chani] = min(stat.rho);
alphaMean = mean(alphaData(:,chani),2);
...
% Save the output
saveName = 'regression_capacity_kcd_peak';
