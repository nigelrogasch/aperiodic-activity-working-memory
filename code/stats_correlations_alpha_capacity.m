...
for compi = 1:size(comparisons,1)
    ...
    if strcmp(comparisons{compi,1},'L02change')
        Behav_data = wmCapacity(:,1)';
    elseif strcmp(comparisons{compi,1},'L04change')
        Behav_data = wmCapacity(:,2)';
    elseif strcmp(comparisons{compi,1},'L06change')
        Behav_data = wmCapacity(:,3)';
    elseif strcmp(comparisons{compi,1},'L02vL04n06change')
    ...
    % Save the output
    saveName = ['correlation_alpha_capacity_',comparisons{compi,1}];
...
for compi = 1:size(comparisons,1)
    ...
    % Load the results
    loadName = ['correlation_alpha_capacity_',comparisons{compi,1}];
