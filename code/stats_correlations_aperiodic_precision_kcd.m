...
% working memory and working memory precision.
%
...
for compi = 1:size(comparisons,1)
    ...
    % Save the output
    saveName = ['correlation_precision_kcd_',comparisons{compi,1},'_',comparisons{compi,2}];
    save([pathStats,saveName,'.mat'],'stat','seedDetails','cohensdPos','cohensdNeg','nParticipants');
...
for compi = 1:size(comparisons,1)
    ...
    % Load the results
    loadName = ['correlation_precision_kcd_',comparisons{compi,1},'_',comparisons{compi,2}];
    ...
    formatSpec = 'Precision %s vs %s: n = %d, pos. p = %.3f, pos. cd = %.2f, neg. p = %.3f, neg. cd = %.2f\n';
