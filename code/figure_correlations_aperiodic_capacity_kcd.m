% Plot the correlations between aperiodic activity and working memory
% capacity.
%
...
statFiles = {'correlation_capacity_kcd_slope_L02Abschange',...
    'correlation_capacity_kcd_slope_L04Abschange',...
    'correlation_capacity_kcd_slope_L06Abschange',...
    'correlation_capacity_slope_L02vL04n06Abschange'};
...
for topox = 1:length(statFiles)
    ...
    % Plot
    scatter(Behav_data,EEG_data.individual(:,eleci),'filled','MarkerFaceColor',cmap(1,:),'MarkerEdgeColor',cmap(1,:));hold on;
    ...
    xlabel(['Working memory capacity (',xLabelIn{topox},')'],'fontsize',12);
...
for topox = 1:length(statFiles)
    colormap(ax.(['tp',num2str(topox)]), flipud(brewermap(12,'PRGn')));
end

%% Save the figure

print(f,'-dpng',[pathFigures,'figure_correlation_aperiodic_capacity_kcd']);
