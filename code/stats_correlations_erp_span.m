% Permutation statistics correlating slow-wave ERP with complex span.
%
...
for compi = 1:size(comparisons,1)
    ...
    % Save the output
    saveName = ['correlation_erp_span_',comparisons{compi,1}];
...
for compi = 1:size(comparisons,1)
    ...
    % Load the results
    loadName = ['correlation_erp_span_',comparisons{compi,1}];
